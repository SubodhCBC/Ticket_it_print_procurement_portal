...
export interface SettingsView {
  ...
  /// Buyers may type a one-off delivery address at checkout (SOW F-18).
  allowCustomDeliveryAddress: boolean

  /// Prices include GST (true) or have it added on the invoice (false).
  pricesIncludeGst: boolean
  /// A money-style string, "15.00".
  gstRatePercent: string

  requirePoNumber: boolean
  poPrefix: string | null
  /// The account-wide PO format, e.g. `PO-####-YY`, or null (SOW F-14).
...
export function toSettingsView(
  ...
  return {
    ...
    allowCustomDeliveryAddress: settings.allowCustomDeliveryAddress,

    pricesIncludeGst: settings.pricesIncludeGst,
    gstRatePercent: settings.gstRatePercent.toFixed(2),

    requirePoNumber: account.requirePoNumber,
    poPrefix: account.poPrefix,
    poFormat: account.poFormat,
