...
import { cacheKey, through } from '../cache/cache.service'
...
// The pure lifecycle file, not the orders service: that would pull its whole
// dependency chain in for four constants.
import {
 ...
 * counts, so a dashboard and a budget can never disagree about what a branch has
 * spent. Reporting only shipped orders would be a different, equally defensible
 * number; having *two* of them in one system is what is not defensible.
 */
 ...
 *
 * Named here rather than in the lifecycle file because the split is a reporting
 * distinction — what the warehouse still has to pick versus what is already with
 * a carrier — and not a rule the state machine enforces.
 */
const AWAITING_DISPATCH_STATUSES: readonly OrderStatus[] = [
  'APPROVED',
  'PROCESSING',
]
 ...
 * though it appears in no historical total.
 */
export interface DashboardReport {
  ...
  /** Spend by category, biggest first (SOW §15 executive spend). */
  readonly topCategories: readonly DimensionRow[]
}
 ...
 * aggregate query answers in milliseconds. When that stops being true, the seam
 * to add is a materialised view, and nothing outside this file changes.
 *
 ...
 * invalidated — see the cache for why a short TTL is preferred to an
 * invalidation web that grows with every feature and fails silently when one
 * path forgets.
 */
/** A report query whose branch filter has been resolved against the caller. */
type Scoped<T extends { siteId?: string }> = Omit<T, 'siteId'> & {
  siteId?: SiteScope
}
...
function cached<T>(
  namespace: string,
  accountId: string,
  query: Scoped<ReportRangeQueryDto> & { by?: string; limit?: number },
  compute: () => Promise<T>
): Promise<T> {
  return through(
    cacheKey(namespace, accountId, {
      from: query.from,
      to: query.to,
      siteId: siteScopeKey(query.siteId),
...
export async function spendSummary(
  ...
  input: ReportRangeQueryDto
): Promise<SpendSummary> {
  const query = await scopedToSites(actor, input)
  return cached(
    'spend-summary',
    resolveAccount(actor, query.accountId),
    query,
    () => computeSpendSummary(actor, query)
  )
}

async function computeSpendSummary(
  actor: AuthenticatedActor,
  query: Scoped<ReportRangeQueryDto>
): Promise<SpendSummary> {
  const accountId = resolveAccount(actor, query.accountId)
  const range = resolveRange(query.from, query.to)
  const previous = previousRange(range)

  const [current, before] = await withTenantScope(accountId, (tx) =>
    Promise.all([
      totals(tx, accountId, range, query.siteId),
      totals(tx, accountId, previous, query.siteId),
    ])
  )
  ...
  const average =
    current.orders === 0
      ...
      : current.spend.dividedBy(current.orders)
  ...
  return {
    ...
    previous: {
      totalSpend: before.spend.toFixed(2),
      orderCount: before.orders,
    },
    spendGrowthPercent: growthPercent(
      Number(current.spend),
      Number(before.spend)
    ),
 ...
 *
 * Empty buckets are filled in rather than omitted: a chart that skips days with
 * no orders draws a straight line through them and silently overstates a quiet
 * week.
 */
export async function spendOverTime(
  ...
  const orders = await withTenantScope(accountId, (tx) =>
    tx.order.findMany({
      where: spendWhere(accountId, range, query.siteId),
      select: { createdAt: true, total: true },
    })
  )

  return { granularity, buckets: bucketise(orders, range, granularity) }
}

/**
 * Folds raw orders into the chart's buckets.
 *
 * Every bucket in the range is prepared first, so a day with no orders is a zero
 * rather than a gap. Shared with the dashboard bundle so the two can never draw
 * different charts from the same window.
 */
function bucketise(
  ...
  const totalsByBucket = new Map<
    string,
    { spend: Prisma.Decimal; orders: number }
  >()
  for (const bucket of bucketsIn(range, granularity)) {
    totalsByBucket.set(bucket, { spend: new Prisma.Decimal(0), orders: 0 })
  }

  for (const order of orders) {
    const bucket = bucketOf(order.createdAt, granularity)
    const entry = totalsByBucket.get(bucket)
  ...
  return [...totalsByBucket.entries()].map(([bucket, entry]) => ({
    bucket,
...
export async function spendBySite(
  actor: AuthenticatedActor,
  input: ReportRangeQueryDto
  ...
  return cached(
    ...
    async () => {
      ...
      return withTenantScope(accountId, (tx) =>
        siteSpendRows(tx, accountId, range, query.siteId)
      )
    }
  )
}
...
async function siteSpendRows(
  ...
  const grouped = await tx.order.groupBy({
    by: ['siteId'],
    where: spendWhere(accountId, range, siteId),
    _sum: { total: true },
    _count: { _all: true },
  })
...
export async function spendByCategory(
  ...
  return cached(
    ...
    async () => {
      ...
      return withTenantScope(accountId, (tx) =>
        categorySpendRows(tx, accountId, range, query.siteId)
      )
    }
  )
}
...
/**
 * Spend per category, against a caller-supplied scope — shared with the
 * dashboard's top categories (SOW §15 executive spend) so both rank by the same
 * rule.
 */
async function categorySpendRows(
  tx: TransactionClient,
  accountId: string | null,
  range: DateRange,
  siteId: SiteScope
  ...
  >`
    ...
    WHERE o."createdAt" >= ${range.from}
      ...
      AND o."status" IN (${Prisma.join(SPEND_STATUSES)})
      ${accountId ? Prisma.sql`AND o."accountId" = ${accountId}` : Prisma.empty}
      ${siteSql(siteId, Prisma.sql`o."siteId"`)}
...
export async function topProducts(
  ...
  return cached(
    ...
    async () => {
      ...
      const range = resolveRange(query.from, query.to)
      const orderBy =
        query.by === 'quantity' ? Prisma.sql`"quantity"` : Prisma.sql`"spend"`
      ...
      const rows = await withTenantScope(
        ...
        WHERE o."accountId" = ${accountId}
          ...
          AND o."createdAt" <  ${range.to}
          AND o."status" IN (${Prisma.join(SPEND_STATUSES)})
          ${siteSql(query.siteId, Prisma.sql`o."siteId"`)}
 ...
 * INVENTORY_VIEW: head office holds the latter, and would otherwise see another
 * customer's contract lines here.
 */
export async function inventoryReport(limit: number): Promise<InventoryReport> {
  ...
  const items = products.map((product) => {
    ...
    return {
      ...
      // Judged on the shelf, not on what is available: this drives reordering,
      // and what needs buying is decided by what is physically there rather than
      // by how much of it is already spoken for.
      status:
  ...
  return {
    ...
    totalUnitsOnHand: items.reduce(
      (total, item) => total + item.stockOnHand,
      0
    ),
    totalUnitsReserved: items.reduce(
      (total, item) => total + item.stockReserved,
      0
    ),
    // The healthy tail is trimmed: this is a "what needs attention" report, and
    // a thousand rows of nothing wrong is not one.
 ...
 * `withTenantScope`. The guard on the route is the whole protection:
 * ACCOUNT_MANAGE is administrator-only and this must never be reachable without
 * it.
 */
 ...
 *
 * Read from the order's frozen `shippingSnapshot`, not from the address row: an
 * address corrected after the fact must not move historical spend between
 * regions. Orders with no region recorded are grouped rather than dropped — a
 * total that silently omits them would not reconcile with the headline.
 */
export async function spendByRegion(
  actor: AuthenticatedActor,
  input: ReportRangeQueryDto
): Promise<readonly DimensionRow[]> {
  const query = await scopedToSites(actor, input)
  ...
  const rows = await withTenantScope(
    ...
    (tx) =>
      tx.$queryRaw<
        { region: string | null; spend: Prisma.Decimal; orders: bigint }[]
      >`
      -- JSON_VALUE rather than the ->> operator, and the whole expression is
      -- repeated in GROUP BY: SQL Server has no ordinal grouping and will not
      -- accept the output alias there either.
      SELECT NULLIF(TRIM(JSON_VALUE(o."shippingSnapshot", '$.region')), '') AS "region",
      ...
      WHERE o."accountId" = ${accountId}
        ...
        AND o."status" IN (${Prisma.join(SPEND_STATUSES)})
        ${siteSql(query.siteId, Prisma.sql`o."siteId"`)}
      GROUP BY NULLIF(TRIM(JSON_VALUE(o."shippingSnapshot", '$.region')), '')
 ...
 * one large one look identical on a revenue chart and mean very different things
 * to a production team.
 */
export async function orderVelocity(
  actor: AuthenticatedActor,
  query: ReportRangeQueryDto
  ...
  const [current, before] = await Promise.all([
    spendOverTime(actor, { ...query, from: range.from, to: range.to }),
    spendOverTime(actor, { ...query, from: previous.from, to: previous.to }),
  ])

  const orders = current.buckets.reduce(
    (total, bucket) => total + bucket.orders,
    0
  )
  const previousOrders = before.buckets.reduce(
    (total, bucket) => total + bucket.orders,
    0
  )
  const days = Math.max(
  ...
  const busiest = current.buckets.reduce<SpendBucket | null>(
    (best, bucket) =>
      best === null || bucket.orders > best.orders ? bucket : best,
    null
  )
 ...
 * still in production have not left the shelf, and counting them would overstate
 * movement in exactly the month a large order was placed and not yet filled.
 *
...
export async function inventoryTurnover(
  ...
  >`
    SELECT p."id"                              AS "productId",
           p."sku"                             AS "sku",
           p."name"                            AS "name",
           -- Conditional, not a bare SUM: with a LEFT JOIN the line row survives
           -- even when the order fails the date and status conditions in the ON
           -- clause, so a plain SUM would count orders from outside the window
           -- and ones that have not shipped. A CASE rather than PostgreSQL's
           -- SUM(...) FILTER (WHERE ...), which SQL Server does not have: it
           -- survived the port and failed every call with error 102. No
           -- backticks in these comments -- this is inside a template literal.
           COALESCE(SUM(CASE WHEN o."id" IS NOT NULL THEN l."quantity" ELSE 0 END), 0) AS "unitsShipped",
    ...
    WHERE p."trackInventory" = 1 AND p."deletedAt" IS NULL
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      turnoverRatio:
        row.stockOnHand === 0
          ? null
          : Math.round((shipped / row.stockOnHand) * 100) / 100,
      // Null when nothing moved: dividing by zero would report "forever", which
      // reads as healthy and is the opposite of what a dead line means.
 ...
 *
 * One grouped query rather than a count per status. Unlike the spend reports
 * this counts *every* status, rejected and cancelled included — the question
 * here is where work is stuck, and the statuses that carry no money are often
...
export async function ordersByStatus(
  ...
  return cached(
    ...
    async () => {
      ...
      return withTenantScope(accountId, (tx) =>
        statusRows(tx, accountId, range, query.siteId)
      )
    }
  )
}
 ...
 *
 * Windowed on `createdAt` and counting every status — unlike the spend reports,
 * rejected and cancelled orders are exactly what a "where is work stuck" view
 * has to show.
 */
async function statusRows(
  ...
  return grouped
    .map((row) => ({
      status: asEnum<OrderStatus>(row.status),
      orders: row._count._all,
      value: (row._sum.total ?? new Prisma.Decimal(0)).toFixed(2),
      // Share of the order *count*, not of the money: this chart is about how
      // many things are where, and a single large order would otherwise make a
      // busy queue look empty.
      sharePercent:
        total === 0 ? 0 : Math.round((row._count._all / total) * 1000) / 10,
    }))
    .sort((a, b) => b.orders - a.orders)
}

/**
 * Every card and chart on a dashboard, in one response.
 *
 ...
 * each call resolves its own date range — so a request that straddles midnight
 * can render cards that disagree with the chart beside them. Here the range is
 * resolved once and every figure is computed against it.
 *
 * Nothing is computed that the individual endpoints do not already compute; this
 * is a different *shape*, not a different set of numbers, which is why the
 * per-dimension helpers are shared rather than reimplemented.
 *
 * The recent-orders table is deliberately absent: it reads order rows rather
 * than aggregates and is guarded by the order permissions, so bundling it would
 * hand order detail to anyone holding REPORT_VIEW alone.
 */
export async function dashboard(
  ...
  if (query.scope === 'platform' && actor.role !== Role.ADMIN) {
    throw new ForbiddenError(
      'The platform-wide dashboard is administrator-only'
    )
  }

  // The only place a null account is produced, and it is produced one line after
  // the check that permits it. Everything downstream reads that null as "every
  // account, no tenant scope".
  const accountId =
    query.scope === 'platform' ? null : resolveAccount(actor, query.accountId)
  ...
  return through(
    ...
    cacheKey('dashboard', accountId ?? 'platform', {
      ...
      categories: query.topCategories,
    }),
    () => computeDashboard(accountId, query)
  )
}

async function computeDashboard(
  accountId: string | null,
  query: Scoped<DashboardQueryDto>
): Promise<DashboardReport> {
  const range = resolveRange(query.from, query.to)
  const previous = previousRange(range)
  const granularity = query.granularity ?? defaultGranularity(range)

  const data = await runScoped(accountId, async (tx) => {
    const [
      current,
      before,
      orders,
      ...
      statusRows(tx, accountId, range, query.siteId),
      queueCounts(tx, accountId, query.siteId),
      query.topSites === 0
        ? Promise.resolve<readonly DimensionRow[]>([])
        : siteSpendRows(tx, accountId, range, query.siteId),
      query.topCategories === 0
        ? Promise.resolve<readonly DimensionRow[]>([])
        : categorySpendRows(tx, accountId, range, query.siteId),
      tx.site.count({
        where: {
          ...
          deletedAt: null,
          ...(accountId ? { accountId } : {}),
          // A dashboard filtered to one branch reports a network of one, not of
          // the whole account. Anything else would put a number on the card that
          // contradicts every other number on the screen.
          ...(query.siteId === undefined
            ? {}
            : typeof query.siteId === 'string'
              ? { id: query.siteId }
              : { id: { in: [...query.siteId] } }),
        },
      }),
  ...
  const trend = bucketise(data.orders, range, granularity)
  const average =
  ...
  // Ties resolve to the earliest, the same rule `orderVelocity` uses.
  ...
  return {
    ...
    spend: {
      ...
      previous: {
        totalSpend: data.before.spend.toFixed(2),
        orderCount: data.before.orders,
      },
      spendGrowthPercent: growthPercent(
        Number(data.current.spend),
        Number(data.before.spend)
      ),
    ...
    topCategories: data.topCategories.slice(0, query.topCategories),
  }
}
...
async function queueCounts(
  ...
  const count = (statuses: readonly OrderStatus[]): number =>
    grouped.reduce(
      (sum, row) =>
        statuses.includes(asEnum<OrderStatus>(row.status))
          ? sum + row._count._all
          : sum,
      0
    )
 ...
 *
 * A null account means the platform scope — no account predicate, and no tenant
 * scope around the query either. `dashboard()` is the only caller that can
 * produce it, and only for an administrator who asked for it explicitly.
 */
function spendWhere(
  accountId: string | null,
  range: DateRange,
 ...
 *
 * The one place the platform scope skips `withTenantScope`, so there is a single
 * line to audit rather than a null check scattered across every query.
 */
function runScoped<T>(
  accountId: string | null,
  fn: (tx: TransactionClient) => Promise<T>
): Promise<T> {
  return accountId === null ? fn(prisma) : withTenantScope(accountId, fn)
}
...
async function totals(
  ...
  const where = spendWhere(accountId, range, siteId)
...
/**
 * Which account a report covers. Exported so an export's About sheet names the
 * same account the figures were computed for, rather than re-deriving the rule.
 */
