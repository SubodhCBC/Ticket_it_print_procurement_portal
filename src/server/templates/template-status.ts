...
export const CUSTOMER_VISIBLE_STATUSES: readonly TemplateStatus[] = [
 ...
 * than ignoring it.
 */
export function editableLayers(
  layers: readonly TemplateLayerLike[]
): readonly TemplateLayerLike[] {
  return layers.filter(
    (layer) => layer.isEditableBySiteUser && isPersonalisableType(layer.type)
  )
}
 ...
 */
export function assertLayersWellFormed(
  layers: readonly TemplateLayerLike[]
): void {
...
export function assertPublishable(template: {
  ...
  /** Null on a design nobody has priced yet. */
  readonly price?: number | null
  readonly unitsPerPack?: number | null
  /**
   * The pack size of the product this design prints onto, when that product is
   * stocked. Undefined for print-on-demand, where there is nothing to reconcile.
   */
  readonly productPackSize?: number | null
}): void {
  if (template.name.trim().length === 0) {
    throw new BusinessRuleError(
      'A template needs a name before it can be published.'
    )
  }

  // Publishing is what puts a design in front of a buyer, so it is the moment
  // it has to have a price. Held here rather than as a NOT NULL column because
  // a half-built draft is an ordinary state and every design that predates
  // template pricing is unpriced through no fault of its own.
  if (template.price == null || template.unitsPerPack == null) {
    throw new BusinessRuleError(
      'This design has no price. Set what a pack costs and how many are in it before publishing.'
    )
  }

  // A stocked product reserves and ships in its own pack size. A design that
  // claimed a different one would sell 100 pieces and reserve 250, and the
  // discrepancy would only surface as a stock count that never reconciles.
  // Print-on-demand products hold no stock, so they have nothing to disagree
  // with and are left to price however the design likes.
  if (
    template.productPackSize != null &&
    template.productPackSize !== template.unitsPerPack
  ) {
    throw new BusinessRuleError(
      `This design says a pack is ${template.unitsPerPack}, but the product it prints onto ships in packs of ` +
        `${template.productPackSize}. Stock is counted in the product's packs, so the two have to agree.`,
      {
        details: {
          unitsPerPack: template.unitsPerPack,
          productPackSize: template.productPackSize,
        },
      }
    )
  }
  ...
  // always described.
  const unnamed = editableLayers(template.layers).filter(
    (layer) => !isNamed(layer)
  )
 ...
 */
/**
 * The one personalisation entry that is not a piece of wording.
 *
 * A buyer may now rework the design itself on the customiser, not only fill in
 * the fields the designer opened up. That artwork belongs to the order being
 * placed - never to the operator's template - so it travels on the line, under
 * this key, as a serialised design document.
 *
 * It is carried, not read. This module decides what a buyer may change about
 * the *published layers*; the document under this key is the buyer's own copy
 * of the design and is not measured against them. Production reads it; nothing
 * here does.
 */
export const ORDER_ARTWORK_KEY = '__artwork'

/**
 * A small picture of the front as the buyer finished it, as a data URL.
 *
 * Carried for the screens that list a line — the basket, checkout, the order —
 * which would otherwise have to rebuild a canvas from the design document to
 * show a thumbnail. Carried, not read, like the artwork; bounded where it
 * enters, in the cart's schema.
 */
export const ORDER_PREVIEW_KEY = '__preview'

/** The picture's cap: a few hundred kilobytes is a thumbnail, not a photograph. */
export const MAX_ORDER_PREVIEW_LENGTH = 600_000

/**
 * Which back design the buyer chose, by its name, for the same screens. Absent
 * when the back prints blank.
 */
export const ORDER_BACK_NAME_KEY = '__backName'

/** Every key that is not a field value, for anything that walks the fields. */
...
/**
 * The cap, mirrored in `lib/design/order-artwork.ts`.
 *
 * Kilobytes for a design document; megabytes once somebody embeds a photograph
 * as a data URI, which belongs in object storage instead. Four million
 * characters refuses the second without troubling the first.
 */
export const MAX_ORDER_ARTWORK_LENGTH = 4_000_000
...
export function acceptCustomisation(
  ...
  const editable = new Map(
    editableLayers(layers).map((layer) => [editableFieldKey(layer), layer])
  )
  ...
  for (const key of Object.keys(submitted)) {
    ...
    const locked = layers.find(
      (candidate) =>
        !candidate.isEditableBySiteUser && editableFieldKey(candidate) === key
    )
  ...
  /* The buyer's own artwork, carried through untouched.
     Every other entry below is rebuilt from the published layers, because a
     value aimed at a locked layer must not survive. This one is not aimed at a
     layer at all: it is the design the buyer will receive, and rebuilding it
     from the operator's would be handing them somebody else's artwork. Its
     size is bounded where it enters, in the cart's own schema. */
  const artwork = Object.prototype.hasOwnProperty.call(
    submitted,
    ORDER_ARTWORK_KEY
  )
    ? submitted[ORDER_ARTWORK_KEY]
    : undefined
  if (typeof artwork === 'string' && artwork.length > 0) {
    accepted[ORDER_ARTWORK_KEY] = artwork
  }

  // The thumbnail and the chosen back's name ride along for the same reason:
  // they describe the buyer's own copy, not a layer of the operator's. A
  // preview has to be an image data URL — anything else is not a picture and
  // is dropped rather than carried to every screen that renders it.
  const preview = Object.prototype.hasOwnProperty.call(
    submitted,
    ORDER_PREVIEW_KEY
  )
    ? submitted[ORDER_PREVIEW_KEY]
    : undefined
  if (typeof preview === 'string' && preview.startsWith('data:image/')) {
    accepted[ORDER_PREVIEW_KEY] = preview
  }
  const backName = Object.prototype.hasOwnProperty.call(
    submitted,
    ORDER_BACK_NAME_KEY
  )
    ? submitted[ORDER_BACK_NAME_KEY]
    : undefined
  if (typeof backName === 'string' && backName.trim().length > 0) {
    accepted[ORDER_BACK_NAME_KEY] = backName.trim()
  }
  ...
  for (const [key, layer] of editable) {
    ...
    const raw = Object.prototype.hasOwnProperty.call(submitted, key)
      ? submitted[key]
      : undefined
    ...
    if (raw === undefined || raw === null || raw === '') {
      if (layer.isRequired) {
        throw new BusinessRuleError(
          `"${layer.label || layer.name}" is required.`,
          {
            details: { fieldKey: key },
          }
        )
    ...
    if (typeof raw !== 'string') {
      throw new BusinessRuleError(
        `"${layer.label || layer.name}" must be text.`,
        {
