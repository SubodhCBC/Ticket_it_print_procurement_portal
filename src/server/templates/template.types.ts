...
export function toFieldViews(
  ...
  return editableLayers(layers).map((layer) => {
    ...
    const label =
      given.length > 0 && !TYPE_NAMES.has(given.toLowerCase())
        ? given
        : humanise(key)
...
export interface TemplateSummaryView {
  ...
  /**
   * Who this belongs to, so a gallery can tell "yours to edit" from "yours to
   * copy" without asking a second endpoint.
   *
   * Null on the operator's own library. The client must not use these to decide
   * what it may do — the server settles that on every write — only to decide
   * which button to show, so that nobody is offered an editor the save will
   * refuse.
   */
  readonly ownerUserId: string | null
  readonly ownerAccountId: string | null
  /** The operator design this was copied from, if it was. */
  ...
  /**
   * What a pack of this design costs and how many pieces it holds. Null on a
   * design nobody has priced yet, which publishing refuses to let happen — so
   * a null here means a draft, and the gallery says so rather than showing a
   * price of nothing.
   */
...
export function toTemplateSummary(
  ...
  return {
    ...
    price: template.price === null ? null : Number(template.price),
...
export interface CustomisableTemplateView {
  ...
  readonly categoryName: string | null
  /**
   * From the frozen version, unlike the product name above.
   *
   * The price is exactly the kind of thing that must not move under a buyer
   * between seeing it and paying it, which is what publishing an immutable
   * version is for. A design repriced this morning does not reprice the basket
   * line someone built last night.
   */
  readonly price: number | null
  readonly unitsPerPack: number | null
  readonly orientation: Template['orientation']
...
export interface TemplateSnapshot {
  ...
  readonly categoryId: string | null
  /**
   * The price this version was published at. Null on a version snapshotted
   * before designs carried prices — those are priced from the catalogue, which
   * is what they were sold at.
   */
  readonly price: number | null
  readonly unitsPerPack: number | null
  readonly theme: string | null
...
export function readSnapshot(value: string | null): TemplateSnapshot {
...
export function toCustomisableView(
  ...
  return {
    ...
    productSku: template.product?.sku ?? null,
    categoryName: template.category?.name ?? null,
    // The version's own columns, not the snapshot's copy: they are what the
    // pricing path reads, so a buyer is quoted from exactly the number a
    // basket will price against rather than from a second copy of it.
    price: version.price === null ? null : Number(version.price),
    unitsPerPack: version.unitsPerPack,
 ...
 *
 * The column is NVARCHAR holding JSON text, not a structured value, so this
 * parses before it narrows. It was validated by the schema on the way in; the
 * narrowing is not a second validation, and a row that somehow holds something
 * else reads as no layers rather than throwing halfway through rendering a
 * gallery of forty.
 */
export function readLayers(value: string | null): readonly TemplateLayerLike[] {
  const parsed = fromJsonOr<unknown>(value, [])
  return Array.isArray(parsed)
    ? (parsed as unknown as readonly TemplateLayerLike[])
    : []
}

/**
 * Reads a JSON object column.
 *
 * Parsing first is the whole job. Under PostgreSQL these were `jsonb` and the
 * client handed back a real object, so a `typeof value === 'object'` test was
 * the correct narrowing. Against NVARCHAR the value is a string, that test is
 * false for every row, and the function silently returned an empty object --
 * type-correct, which is why nothing caught it, and wrong on every read.
 */
