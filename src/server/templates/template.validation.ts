...
export const TemplateLayerSchema = z
  .object({
    ...
    /**
     * The layer's payload: the words on a text layer, the scan target on a QR
     * code -- and, on an image layer, the picture itself.
     *
     * That last one is why this is four million and not twenty thousand. The
     * builder reads a chosen file with `FileReader.readAsDataURL`, so an image
     * layer's content is a `data:image/...;base64,...` URL, and the same bytes
     * reach this endpoint three times over: here, in `design`, and inside
     * `canvasJson` (fabric's `toJSON` embeds each image's `src`). A twenty
     * thousand character cap here rejected the save while the other two
     * accepted the identical data, so it was not keeping base64 out of the
     * database -- it was only refusing to let a designer use an image. The
     * number matches `canvasJson` deliberately: one payload, one ceiling.
     *
     * This is the interim arrangement, not the destination. Images belong in
     * object storage reached by key -- the presign/upload/register path already
     * exists and the thumbnail already takes it -- and the plan is for that
     * storage to be the DAM. Until then a template carries its pictures inside
     * it, which also means every version snapshot and every customer copy
     * carries them again.
     */
    content: z.string().max(4_000_000).default(''),
  })
  .passthrough()
...
const templateBody = {
  ...
  /**
   * What one pack of this design costs, and how many pieces are in a pack.
   *
   * Optional here because a draft is allowed to be half-built; `assertPublishable`
   * is what refuses to publish an unpriced design. Accepting them is also gated
   * on PRICING_MANAGE in the service — a customer editing their own copy may
   * send these fields and they are ignored, rather than the whole save being
   * rejected over a field their builder had no business filling in.
   *
   * Zero is a valid price (a design given away with a campaign); zero pieces
   * per pack is not, because the storefront divides by it to print "2.50 each".
   */
  price: z.coerce.number().min(0).max(1_000_000).optional(),
  unitsPerPack: z.coerce.number().int().min(1).max(10_000_000).optional(),
...
export const UpdateTemplateSchema = z
  .object({
    ...
    price: templateBody.price,
    unitsPerPack: templateBody.unitsPerPack,
...
export const SetTemplateVisibilitySchema = z
  ...
  .refine(
    (body) => body.visibility === 'ALL_ACCOUNTS' || body.accountIds.length > 0,
    {
      message:
        'A RESTRICTED template with no accounts listed would be visible to nobody.',
      path: ['accountIds'],
    }
  )

export type SetTemplateVisibilityDto = z.infer<
  typeof SetTemplateVisibilitySchema
>
...
/**
 * Attaches a file from the document library to a template.
 *
 * The library is where artwork lives now, so a template asset names a file in it
 * rather than an object the caller uploaded to storage itself. Upload to
 ...
 * copies the file and reads all three off it. The two-step presign flow this
 * replaces had to take the caller's word for the size, and a caller that lied
 * got a row that disagreed with the object.
 */
export const AttachTemplateAssetSchema = DamFileRefSchema.extend({
