...
export function ruleMatches(
  ...
  if (rule.minTotalCents !== null && facts.totalCents < rule.minTotalCents)
    return false
  if (rule.siteId !== null && rule.siteId !== facts.siteId) return false
  if (rule.requesterRole !== null && rule.requesterRole !== facts.requesterRole)
    return false
  if (rule.categoryId !== null && !facts.categoryIds.includes(rule.categoryId))
    return false
  return true
}
...
export function planApproval(
  ...
  return steps.sort(
    (a, b) => a.tier - b.tier || a.ruleId.localeCompare(b.ruleId)
  )
}
...
export function canDecideStep(
  ...
  if (step.approverRole)
    return step.approverRole === actor.role || actor.role === 'ADMIN'
  return false
}
...
export function evaluateProgress(steps: readonly StepState[]): RequestProgress {
  ...
  if (rejected) {
    return {
      outcome: 'REJECTED',
      currentTier: rejected.tier,
      skipTiersAbove: rejected.tier,
    }
  }
  ...
  const tiers = [...new Set(steps.map((step) => step.tier))].sort(
    (a, b) => a - b
  )
  ...
  for (const tier of tiers) {
    ...
    const settled = inTier.every(
      (step) => step.status === 'APPROVED' || step.status === 'SKIPPED'
    )
    if (!settled)
      return { outcome: 'PENDING', currentTier: tier, skipTiersAbove: null }
  }
