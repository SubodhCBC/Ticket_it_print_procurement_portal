import type { LineImage } from '../orders/line-images'
...
export interface ApprovalRequestView {
  ...
  /** What is being approved, line by line, with a picture of each. */
...
export interface ApprovalLineView {
  ...
  /** The design printed on it, when it is one. */
  ...
  /**
   * The artwork. `source` BUYER_PREVIEW is the design as the buyer finished it,
   * fetched from `previewPath` with the caller's credentials; TEMPLATE and
   * PRODUCT are the unpersonalised design and the blank stock, at `imageUrl`,
   * and should be labelled as such rather than shown as a proof.
   */
...
const NO_IMAGE: LineImage = {
  source: 'NONE',
  previewPath: null,
  imageUrl: null,
}

export function toApprovalRequestView(
  request: FullApprovalRequest,
  images: ReadonlyMap<string, LineImage> = new Map()
): ApprovalRequestView {
  return {
    id: request.id,
    ...
    lines: request.order.lines.map((line) => ({
      id: line.id,
      sku: line.sku,
      name: line.name,
      variantSku: line.variantSku,
      templateId: line.templateId,
      templateName: line.template?.name ?? null,
      quantity: line.quantity,
      uom: line.uom,
      unitPrice: line.unitPrice.toFixed(2),
      lineTotal: line.lineTotal.toFixed(2),
      notes: line.notes,
      image: images.get(line.id) ?? NO_IMAGE,
    })),
