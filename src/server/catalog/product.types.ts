...
import { fromJsonOr, fromStringList } from '../db/json-column'
...
export interface ProductOptionView {
   ...
   * value. Values absent from the map add nothing.
   */
  readonly valuePrices: Readonly<Record<string, string>>
...
export interface ProductView {
  ...
  /** STANDARD, ZERO_RATED or EXEMPT (SOW F-06). */
...
export function toProductView(
  ...
  return {
    ...
    tags: fromStringList(product.tags),
    options: product.options.map(toOptionView),
    variants: product.variants.map((variant) => ({
      id: variant.id,
      sku: variant.sku,
      attributes: fromJsonOr<Record<string, string>>(variant.attributes, {}),
    ...
    assets: product.assets.map((asset) => ({
      ...
      ...(assetUrls[`${asset.id}:thumbnail`]
        ...
        : {}),
      ...(assetUrls[`${asset.id}:preview`]
        ? { previewUrl: assetUrls[`${asset.id}:preview`] }
        : {}),
    })),
...
export type ImportJobSummaryRow = Omit<
  CatalogImportJob,
  'payload' | 'results'
> & {
  results?: unknown
...
export function toImportJobView(
  job: ImportJobSummaryRow,
  ...
  return {
    ...
    ...(includeResults
      ? { results: fromJsonOr<unknown>(job.results as string | null, null) }
      : {}),
  }
}
