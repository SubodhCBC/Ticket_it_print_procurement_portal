import { z } from 'zod'
import { PoFormatField } from '@/server/cart/po-format.validation'
import {
...
const AddressInput = z.object({
  ...
  /**
   * An id from `GET /api/v1/shipping/addresses`. When sent, the address is
   * validated through NZ Post and stored with NZ Post's lines, DPID and rural
   * flag (SOW F-16); the typed lines are replaced, except a typed `line2`.
   */
...
export const CreateSiteSchema = z.object({
  ...
  /** This branch's own format; wins over the account's. Blank clears it. */
...
/** Validates a saved address against a ParcelAddress result after the fact. */
export const ValidateSiteAddressSchema = z.object({
  nzPostAddressId: z.string().trim().min(1).max(64),
})
export type ValidateSiteAddressDto = z.infer<typeof ValidateSiteAddressSchema>
