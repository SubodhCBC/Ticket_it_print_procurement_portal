...
function layout(options: {
  ...
  return [
    ...
    'This is an automated message from the Print Procurement Portal. Please do not reply to it.',
    '</p></div>',
  ].join('')
}
...
export function renderInvitationMail(input: InvitationMailInput): RenderedMail {
  ...
  const intro = input.isExternal
    ? `You have been given limited access to the ${input.accountName} portal on Print Procurement Portal.`
    : `You have been invited${invitedBy} to the ${input.accountName} portal on Print Procurement Portal.`
...
export function renderPasswordResetMail(
  ...
  return {
    subject: 'Reset your Print Procurement Portal password',
    html: layout({
      heading: `Hello ${input.firstName},`,
      body: [
...
export interface OrderSummaryInput {
  ...
  /** The buyer's own reference (SOW F-14). Free text, escaped like the rest. */
...
function orderFacts(order: OrderSummaryInput): string {
  ...
  // The buyer typed this, so it is escaped on the way into the markup below
  // like every other value — never trusted as HTML.
  if (order.customerReference) {
    rows.push(['Customer reference', order.customerReference])
  }
...
function orderFactsText(order: OrderSummaryInput): string {
  return [
    ...
    ...(order.customerReference
      ? [`Customer ref:   ${order.customerReference}`]
      : []),
  ].join('\n')
}
...
export function renderOrderPlacedMail(
  input: OrderPlacedMailInput
): RenderedMail {
...
export function renderApprovalPendingMail(
  input: ApprovalPendingMailInput
): RenderedMail {
  return {
...
export interface ApprovalDecidedMailInput {
  ...
  /**
   * One tier of several cleared, and the order gone on to the next (SOW §10
   * acceptance: "the requester is notified on every approval decision").
   */
  readonly interim?: {
    readonly decidedTier: number
    readonly nextTier: number
  } | null
  readonly orderUrl: string
}

export function renderApprovalDecidedMail(
  input: ApprovalDecidedMailInput
): RenderedMail {
  // Only an approval can be interim: a refusal or a request for changes ends
  // the round, whatever tier it came from.
  const interim = input.decision === 'APPROVED' ? input.interim : null

  const wording = interim
    ? {
        subject: `Order ${input.order.orderNumber} passed approval step ${interim.decidedTier}`,
        heading: 'Your order has passed one approval step',
        body:
          `${input.decidedByName} approved your order at step ${interim.decidedTier}. ` +
          `It is not approved yet: it has gone to step ${interim.nextTier} for the next ` +
          'decision, and you will be emailed again when that is made.',
      }
    : {
        APPROVED: {
...
export function renderOrderDispatchedMail(
  input: OrderDispatchedMailInput
): RenderedMail {
  ...
  return {
    ...
    html: layout({
      ...
      body: [
        ...
        ...input.items.map(
          (item) =>
            `<p style="margin:0 0 4px;font-size:14px">${escapeHtml(line(item))}</p>`
        ),
        '</div>',
