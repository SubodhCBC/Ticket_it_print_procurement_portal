...
const OrderSummary = z.object({
  ...
  // Optional, so a job queued before this field existed still parses on the way
  // out of Redis rather than failing as an unknown shape.
...
export const ApprovalDecidedJobSchema = BaseRecipient.extend({
  ...
  /**
   * Set when this decision cleared one tier of several and the order has moved
   * on to the next, rather than resolving the request. Absent on jobs queued
   * before it existed, which were all final decisions.
   */
  interim: z
    .object({
      decidedTier: z.number().int().min(1),
      nextTier: z.number().int().min(2),
    })
    .nullish(),
})
