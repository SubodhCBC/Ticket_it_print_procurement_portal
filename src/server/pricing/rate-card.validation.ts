...
const Percent = z
  ...
  .regex(
    /^\d{1,3}(\.\d{1,2})?$/,
    'Expected a percentage such as "15" or "17.24"'
  )
...
export const RateCardItemSchema = z
  ...
  .refine(
    (value) =>
      new Set(value.tiers.map((tier) => tier.minQuantity)).size ===
      value.tiers.length,
...
export type ListRateCardItemsQueryDto = z.infer<
  typeof ListRateCardItemsQuerySchema
>
  ...
  lines: z
    .array(
      z.object({
        ...
        /**
         * The published design this line is personalised from, when it has
         * one. A line naming a version is priced from that version's frozen
         * price and the rate card is not consulted — see `priceForTemplate`.
         */
        templateVersionId: z.string().trim().min(1).max(64).optional(),
        /**
         * The chosen configuration. On a design-priced line its option values
         * can each add to the pack price — a laminated stock costs more than a
         * plain one — so the variant is part of what the price depends on.
         */
