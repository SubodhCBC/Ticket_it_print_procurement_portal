...
export function toRateCardView(
  ...
  return {
    ...
    ...(items
      ? { items: items.map((item) => toRateCardItemView(item, card)) }
      : {}),
  }
}
...
export function toRateCardItemView(
  ...
  card: {
    defaultDiscountPercent: { toFixed(digits: number): string }
    id: string
    name: string
  }
): RateCardItemView {
...
export function toActiveRateCardView(
  card: PricingRateCard | null
): ActiveRateCardView | null {
  if (!card) return null
  return {
    id: card.id,
    name: card.name,
