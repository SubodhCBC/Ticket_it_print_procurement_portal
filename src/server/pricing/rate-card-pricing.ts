...
export type PriceSource =
  ...
  /**
   * The published design said so, and nothing else was consulted.
   *
   * Outside the six-rule ladder above rather than at the top of it: the others
   * describe a catalogue price being discounted, and this one replaces the
   * catalogue price entirely. A line priced this way has no rate card, no
   * ladder and no saving — see `priceForTemplate`.
   *
   * Historical rows keep the value they were written with. An order placed
   * before templates carried prices still reads CONTRACT_FIXED_PRICE, and
   * should: it says what actually happened.
   */
  | 'TEMPLATE_PRICE'
 ...
 * has a rate card.
 */
export function priceForContract(
  input: ContractPriceInput
  ...
  return {
    ...
    source:
      contract?.source ??
      (catalog.appliedTier ? 'CATALOG_VOLUME_TIER' : 'CATALOG_BASE'),
...
/**
 * Prices one line from the design it was personalised from.
 *
 * The whole six-rule ladder above is skipped, and that is the point: a template
 * price is what the operator decided this design costs, and a rate card
 * negotiated against a *product* has nothing to say about it. A buyer who
 * personalises a design pays what the design costs, however far they move the
 * text about — which is the rule the customiser is built around.
 *
 * Returns the same `ContractPriceBreakdown` every other price returns, so the
 * basket view, the order snapshot and every DTO downstream keep working with no
 * knowledge that a second kind of price exists. The catalogue fields are set to
 * the template price rather than to the product's, so `savingCents` is zero
 * rather than a discount nobody offered: there is no counterfactual here, and
 * inventing one out of `Product.basePrice` would put a struck-through number on
 * the screen that no longer means anything.
 */
export function priceForTemplate(input: {
  readonly unitPriceCents: number
  readonly quantity: number
}): ContractPriceBreakdown {
  assertPositiveInteger(input.quantity, 'quantity')
  assertNonNegativeInteger(input.unitPriceCents, 'unitPriceCents')

  const lineTotalCents = input.unitPriceCents * input.quantity

  return {
    quantity: input.quantity,
    baseUnitPriceCents: input.unitPriceCents,
    source: 'TEMPLATE_PRICE',
    discountPercent: 0,
    ...
    rateCardId: null,
    rateCardName: null,
    catalogUnitPriceCents: input.unitPriceCents,
    catalogLineTotalCents: lineTotalCents,
    savingCents: 0,
    aboveCatalogPrice: false,
  }
}
...
export function priceLadderForContract(
  ...
  for (const tier of input.card?.item?.tiers ?? [])
    thresholds.add(tier.minQuantity)
...
function assertPositiveInteger(value: number, field: string): void {
  if (!Number.isInteger(value) || value < 1) {
    throw new BusinessRuleError(
      `${field} must be a whole number of at least 1`,
      {
        details: { [field]: value },
      }
    )
...
function assertNonNegativeInteger(value: number, field: string): void {
  if (!Number.isInteger(value) || value < 0) {
    throw new BusinessRuleError(
      `${field} must be a whole number of at least 0`,
      {
        details: { [field]: value },
      }
    )
