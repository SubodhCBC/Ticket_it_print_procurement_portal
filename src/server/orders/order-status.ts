...
export const AWAITING_APPROVAL_STATUSES: readonly OrderStatus[] = [
  OrderStatus.PENDING_APPROVAL,
]
 ...
 */
export const OPEN_STATUSES: readonly OrderStatus[] = Object.values(
  OrderStatus
).filter(
  (status) =>
    status !== OrderStatus.DRAFT && !TERMINAL_STATUSES.includes(status)
)
...
export function assertTransition(from: OrderStatus, to: OrderStatus): void {
  if (from === to) {
    throw new BusinessRuleError(`This order is already ${to}.`, {
      details: { status: to },
    })
  }
