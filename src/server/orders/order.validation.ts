...
export const PlaceOrderSchema = z.object({
   ...
   *
   * Makes the submit safe to repeat. Sent again after a timeout or a double
   * click, and the order that basket already became comes back with 200 rather
   * than a second order or an "empty basket" error. If the buyer's open basket
   * is no longer this one, the request is refused with 409 rather than placing
   * a basket they did not review. Optional, but without it a repeated submit
   * after the first succeeded answers 422 (the basket is empty).
   */
  cartId: z.string().trim().min(1).max(64).optional(),
  /**
   * Instructions for whoever delivers — "leave at the back dock". Carried onto
   * the order and the label. Required when the account's settings say so.
   */
...
export const ChangeOrderStatusSchema = z
  .object({
    ...
    /**
     * Replaces the order's delivery instructions, on any transition — the
     * fulfilment screen's "dispatch notes". Omit to leave them as they are.
     */
  ...
  .refine(
    (value) =>
      value.status !== 'CHANGES_REQUESTED' || (value.reason?.length ?? 0) > 0,
