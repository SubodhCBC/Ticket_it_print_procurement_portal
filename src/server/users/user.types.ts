...
import type { Role, UserType } from '@/server/context/request-context'
...
export interface UserSummaryView {
  ...
  /** The user's own spend ceiling per billing period; null for none. */
  readonly monthlyBudgetCap: string | null
...
export function toUserSummaryView(user: UserWithSites): UserSummaryView {
  return {
    ...
    monthlyBudgetCap: user.monthlyBudgetCap?.toFixed(2) ?? null,
...
export function toInvitationView(invitation: Invitation): InvitationView {
  return {
    ...
    role: asEnum<Role>(invitation.role),
    userType: asEnum<UserType>(invitation.userType),
