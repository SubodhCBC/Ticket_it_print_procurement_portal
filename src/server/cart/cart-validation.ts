...
export const CartIssueCode = {
  ...
  /** The reference does not fit the site's or account's PO format (SOW F-14). */
  PO_FORMAT_MISMATCH: 'PO_FORMAT_MISMATCH',
  BUDGET_EXCEEDED: 'BUDGET_EXCEEDED',
  /** The order would take the buyer past their own monthly limit (AD-4). */
  ...
  /**
   * The bill-to the buyer picked has since been deleted, retyped or moved to
   * another branch. Blocking, because silently billing a default instead would
   * send the invoice somewhere the buyer did not choose.
   */
  BILLING_ADDRESS_NOT_AVAILABLE: 'BILLING_ADDRESS_NOT_AVAILABLE',
  SHIPPING_METHOD_REQUIRED: 'SHIPPING_METHOD_REQUIRED',
  PAYMENT_METHOD_REQUIRED: 'PAYMENT_METHOD_REQUIRED',
  TERMS_NOT_ACCEPTED: 'TERMS_NOT_ACCEPTED',
  DELIVERY_DATE_IN_PAST: 'DELIVERY_DATE_IN_PAST',
  /**
   * The account requires delivery instructions and the order was placed without
   * any. Raised by `POST /orders`, where the instructions arrive — the basket
   * does not hold them — so it never appears on a cart validation.
   */
  DELIVERY_NOTES_REQUIRED: 'DELIVERY_NOTES_REQUIRED',
} as const
 ...
 *
 * `enforceMoq` is the account's "hold lines at their minimum order quantity"
 * setting. Off, the shortfall is still reported, but the line is ordered as
 * typed: `orderableQuantity` stays at the buyer's quantity, so nothing rounds
 * it, prices it higher, or offers an adjustment to accept.
 */
export function checkLine(
      ...
      issues: [
        {
          ...
          message:
            'This product is no longer available. Remove the line to continue.',
          lineId,
        },
      ],
      warnings: [],
  ...
  const orderableQuantity = enforceMoq
    ? roundToOrderable(quantity, product.moq, product.orderMultiple)
    : quantity

  if (
    !enforceMoq &&
    !isOrderableQuantity(quantity, product.moq, product.orderMultiple)
  ) {
    const suggested = roundToOrderable(
      quantity,
      product.moq,
      product.orderMultiple
    )
    const below = quantity < product.moq
    warnings.push({
      code: below
        ? CartIssueCode.QUANTITY_BELOW_MOQ
        : CartIssueCode.QUANTITY_NOT_MULTIPLE,
      message: below
        ? `"${product.name}" has a minimum order of ${product.moq}. You can still order ${quantity}.`
        : `"${product.name}" is sold in multiples of ${product.orderMultiple} above ${product.moq}. You can still order ${quantity}.`,
      lineId,
      details: {
        moq: product.moq,
...
export function checkCheckoutDetails(
  ...
  if (!facts.shippingAddressId) {
    ...
    issues.push({
      ...
      message:
        'The selected delivery address is no longer available. Choose another.',
      lineId: null,
    })
  }

  if (!facts.shippingMethod) {
    issues.push({
      code: CartIssueCode.SHIPPING_METHOD_REQUIRED,
      message: 'Choose how this order should be delivered.',
      lineId: null,
    })
  }
  ...
  if (
    facts.requestedDeliveryDate &&
    facts.requestedDeliveryDate < startOfUtcDay(facts.now)
  ) {
