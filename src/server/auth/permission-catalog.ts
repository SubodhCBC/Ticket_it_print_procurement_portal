...
export interface PermissionDescriptor {
   ...
   * administrator looking for who may issue an invoice can type "invoice" rather
   * than having to already know the answer is `BILLING_MANAGE`.
   */
 ...
 * PUT. Departures from it are per-user grants.
 *
 ...
 */
export function permissionCatalog(): PermissionCatalogView {
  return {
    ...
    roles: [
      ...
      },
      {
        ...
        permissions: sorted(
          basePermissionsFor(Role.HEAD_OFFICE, UserType.EXISTING)
        ),
      },
      {
        role: Role.SITE_USER,
        ...
        permissions: sorted(
          basePermissionsFor(Role.SITE_USER, UserType.EXISTING)
        ),
      },
      {
        role: 'EXTERNAL',
        ...
        permissions: sorted(
          basePermissionsFor(Role.SITE_USER, UserType.EXTERNAL)
        ),
      },
    ],
  }
...
function sorted(permissions: ReadonlySet<Permission>): Permission[] {
 ...
 *
 * The catalog is already grouped by prefix — APPLICATION_*, DAM_*, ORDER_* — so
 * a separate group column would be a second thing to keep in step with the
...
function groupOf(permission: Permission): string {
  ...
  if (permission.startsWith('ORDER_') || permission === Permission.APPROVAL_ACT)
    return 'Orders'
  ...
  if (permission.startsWith('REPORT_') || permission.startsWith('AUDIT_'))
    return 'Reporting'
