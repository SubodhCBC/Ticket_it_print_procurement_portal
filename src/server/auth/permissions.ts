import { Role, UserType } from '../context/request-context'
...
export const Permission = {
  ...
  /// Download a product's artwork and reference files (SOW F-10: "downloadable
  /// subject to role permission"). Product photographs are not gated by it —
  /// they are what the catalogue shows — only ARTWORK and SPEC_SHEET files.
  CATALOG_FILE_DOWNLOAD: 'CATALOG_FILE_DOWNLOAD',
  PRICING_VIEW: 'PRICING_VIEW',
  PRICING_MANAGE: 'PRICING_MANAGE',

  // --- Ordering --------------------------------------------------------------
  ORDER_CREATE: 'ORDER_CREATE',
  /// Type a one-off delivery address at checkout (SOW F-18), where the account
  /// allows it. The account switch is the main control; this lets an
  /// administrator withhold it from one buyer.
  ORDER_CUSTOM_DELIVERY_ADDRESS: 'ORDER_CUSTOM_DELIVERY_ADDRESS',
...
export const PERMISSION_DESCRIPTIONS: Record<Permission, string> = {
  ...
  [Permission.APPLICATION_CREATE]:
    'Create records in the general parts of the portal.',
  [Permission.APPLICATION_EDIT]:
    'Edit records in the general parts of the portal.',
  [Permission.APPLICATION_DELETE]:
    'Delete records in the general parts of the portal.',

  // --- Document Access Management -------------------------------------------
  [Permission.DAM_VIEW]: 'Browse the document library and see what it holds.',
  [Permission.DAM_UPLOAD]:
    'Add artwork, proofs and other files to the library.',
  [Permission.DAM_DOWNLOAD]:
    'Download files from the library, including print-ready artwork.',
  ...
  [Permission.CATALOG_VIEW]:
    'Browse the product catalogue and open a product page.',
  [Permission.CATALOG_MANAGE]:
    'Create and edit products, categories, options and variants; publish and retire them.',
  [Permission.CATALOG_FILE_DOWNLOAD]:
    'Download the artwork and reference files attached to a product, such as print-ready ' +
    'artwork and specification sheets.',
  ...
  [Permission.ORDER_CUSTOM_DELIVERY_ADDRESS]:
    'Type a one-off delivery address at checkout instead of choosing a saved one, where the account allows it.',
  [Permission.ORDER_VIEW_OWN]:
    'See the orders you placed yourself, and nobody else’s.',
  [Permission.ORDER_VIEW_SITE]:
    'See every order for the branches you are attached to.',
  [Permission.ORDER_VIEW_ACCOUNT]:
    'See every order in the account, across all of its branches.',
  [Permission.ORDER_CANCEL]:
    'Cancel an order that has not yet gone into production.',
  ...
  [Permission.APPROVAL_ACT]:
    'Approve, reject or send back an order that is waiting on a decision.',
  ...
  [Permission.BILLING_VIEW]:
    'See invoices, billing periods and what is still unbilled.',
  [Permission.BILLING_MANAGE]:
    'Generate, issue, void and mark invoices paid; run the monthly billing.',
  [Permission.INVENTORY_VIEW]:
    'See stock on hand and what is reserved against orders.',
  [Permission.INVENTORY_MANAGE]:
    'Adjust stock levels, reconcile counts and receive deliveries into the warehouse.',
  [Permission.REPORT_VIEW]:
    'Open the reporting dashboards — spend, orders, top products. A user without ' +
    'account-wide order access sees their own branches only.',
  [Permission.AUDIT_VIEW]:
    'Read the audit trail of who changed what, and when.',
...
const SITE_USER_PERMISSIONS: readonly Permission[] = [
  ...
  // Granted with ordering, so the account setting is what switches it on; an
  // administrator can still DENY it to one buyer. External users do not get it —
  // their list is closed, see EXTERNAL_PERMISSIONS.
  Permission.ORDER_CUSTOM_DELIVERY_ADDRESS,
  Permission.ORDER_VIEW_OWN,
  Permission.ORDER_VIEW_SITE,
  Permission.TEMPLATE_USE,
  Permission.DAM_VIEW,
  Permission.DAM_DOWNLOAD,
  // Their own branch's reports (SOW §15: "Site User sees own site only"). The
  // scope is decided in the reports service from their order permissions, not
  // by what they ask for — see `reportScope`.
  Permission.REPORT_VIEW,
]
...
const HEAD_OFFICE_PERMISSIONS: readonly Permission[] = [
  ...
  Permission.BILLING_VIEW,
  Permission.INVENTORY_VIEW,
  // Not in the site-user baseline: print-ready artwork is the brand's, and a
  // branch orders from it rather than taking it away. An administrator can
  // ALLOW it to one buyer.
  Permission.CATALOG_FILE_DOWNLOAD,
  Permission.AUDIT_VIEW,
