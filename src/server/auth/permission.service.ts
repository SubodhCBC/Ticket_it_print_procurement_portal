 ...
 *
 * Keyed on the RequestContext object rather than on a user id so the entry dies
 * with the request, and so two requests from the same user never share a
 ...
 * request alive.
 */
const perRequestCache = new WeakMap<
  RequestContext,
...
export async function resolvePermissions(
  ...
  if (!context) return loadPermissions(actor)

  const cached = perRequestCache.get(context)
  if (cached) return cached

  const pending = loadPermissions(actor)
...
async function loadPermissions(
  ...
  const rows = await prisma.userPermissionGrant.findMany({
    where: {
      userId: actor.userId,
      // Redundant next to userId, and deliberately so: application-level tenant
      // scoping is the first line of defence, and a grant row that somehow
      // pointed at another account must not be honoured.
  ...
  for (const row of rows) {
    // A grant naming a permission this build no longer knows about is ignored
    // rather than trusted. Renaming a permission leaves exactly this kind of
    // orphan behind, and the safe reading of an unknown grant is "no".
