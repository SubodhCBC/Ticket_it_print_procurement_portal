...
import { APPROVAL_ACTIVITY } from '@/server/reports/report-tables'
...
/**
 * GET /api/v1/reports/approvals/activity.xlsx
 *
 * Every approval decision in the window: who, which order, the outcome and the cycle time.
 *
 * The same rows as `GET /api/v1/reports/approvals/activity`, with the columns declared
...
export const GET = route(
  ...
  async ({ request, actor, requestId }) => {
    ...
    const report = await approvalActivity(actor, query)
    ...
    const body = await renderXlsx(APPROVAL_ACTIVITY, rows, context)
