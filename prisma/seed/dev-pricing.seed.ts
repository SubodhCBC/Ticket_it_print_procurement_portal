...
const ACTIVE_CARD = {
  ...
  notes:
    'Group agreement, renewed annually. 12% off list with negotiated print lines.',
...
export async function seedDevPricing(prisma: PrismaClient): Promise<void> {
  ...
  if (!account) {
    console.log(
      '[seed] pricing: demo account missing — run the user seed first. Skipped.'
    )
    return
  }
  ...
  const renewalFrom = new Date(
    Date.UTC(effectiveFrom.getUTCFullYear() + 1, 0, 1)
  )
